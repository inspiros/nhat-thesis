...
\subsection{3D CNN based clip-level feature extraction}
    ...
    We utilize the network pre-trained on Sports-1M and fine-tuned on Kinetics dataset. To apply in our framework of hand gestures, we apply transfer learning technique on each data stream. Detail of training the network will be presented in section \ref{sec:experimentalresult}. The feature vector of 4096 dimensions extracted from FC\-6 layer will be served for training and testing classifiers in further steps.
